...
Below are some notes on how to build Globe Core for Windows.

The options known to work for building Globe Core on Windows are:
...
Note that for WSL the Globe Core source path MUST be somewhere in the default mount file system, for
example /usr/src/globe, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\globe`, for example:

    make install DESTDIR=/mnt/c/workspace/globe
