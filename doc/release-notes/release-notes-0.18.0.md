Globe Core version 0.18.0 is now available.
...
`/Applications/Globe-Qt` (on Mac) or `globed`/`globe-qt` (on
Linux).
...
recommended to use Globe Core on unsupported systems.
...
`contrib/init/globed.service`, it has been changed to use
`/var/lib/globed` as the data directory instead of
`~globe/.globe`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/globed` will
exist has enough space (check using `df -h /var/lib/globed`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/globe/globe/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://globecore.org/en/contact/) the Globe Core
...
The systemd init file (`contrib/init/globed.service`) has been changed
to use `/var/lib/globed` as the data directory instead of
`~globe/.globe`. This change makes Globe Core more consistent with
...
`datadir` option in `/etc/globe/globe.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/globe/globe.conf`.


Documentation
-------------

- A new short [document](https://github.com/globe/globe/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/globe/globe/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/globe/globe/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/globe/globe/blob/master/doc/globe-conf.md)
  about the `globe.conf` file describes how to use it to configure
  Globe Core.

- A new document introduces Globe Core's BIP174 [Partially-Signed
  Globe Transactions
  (PSBT)](https://github.com/globe/globe/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/globe/globe/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Globe-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Globe Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/globe/globe/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/globe/globe/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Globe Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Globe Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/globe/globe/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/globe/globe/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart globed with the `-deprecatedrpc=generate`
...
- A new `globe-wallet` tool is now distributed alongside Globe
...
This section describes planned changes to Globe Core that may affect
other Globe software and services.

- Since version 0.16.0, Globe Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Globe Core 0.20 (expected about
  a year after 0.18), Globe Core will default to native segwit
  ...
  to bech32 addresses, and if the Globe Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Globe Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Globe Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Globe Core
...
- A sub-project of Globe Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Globe Core.  See their [project
  page](https://github.com/globe-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Globe Core's own implementation, although entropy
  gathered by Globe Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Globe Core a
...
- On macOS, Globe Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `globe_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libglobe_server.a` (jnewbery)
...
- #13926 [Tools] globe-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like globed (MarcoFalke)
...
- #14521 qt, docs: Fix `globe-qt -version` output formatting (hebasto)
...
- #11625 Add GlobeApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libglobe_server` (laanwj)
...
- #15154 Configure: globe-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/globe-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Globe Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libglobe_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in globe-util-test.py (isghe)
...
- #12775 Integration of property based testing into Globe Core (Christewart)
...
- #12246 Bugfix: Only run globe-tx tests when globe-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_globe` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_globe` (MarcoFalke)
...
- #14186 globe-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_globe.conf` message on startup if conf file exists (leishman)
...
- #12255 Update globe.service to conform to init.md (dongcarl)
...
- #14497 Add doc/globe-conf.md (hebasto)
...
- #15354 Add missing `globe-wallet` tool manpages (MarcoFalke)
...
- #15765 correct globeconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support globe folder on OSX (JimmyMow)
