# User-space, Statically Defined Tracing (USDT) for Globe Core

Globe Core includes statically defined tracepoints to allow for more
...
**Note**: Globe Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Globe Core
...
Multiple tools can list the available tracepoints in a `globed` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Globe Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/globed
stap net        outbound_message 0x0000000000107c05 /src/globed
stap validation block_connected  0x00000000002fb10c /src/globed
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Globe Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/globed | grep NT_STAPSDT -A 4 -B 2
