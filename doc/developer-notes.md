...
- [Development guidelines](#development-guidelines)
    - [General Globe Core](#general-globe-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Globe Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while globed is running with the `logging`
RPC.  For instance, launching globed with `-debug` or `-debug=1` will turn on
...
with "play globes" on a test network.
...
Globe Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run globed.

You can then use the `-debug=lock` configuration option at globed startup or
`globe-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `globe-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/globe/globe/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_globe
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_globe --log_level=test_suite
$ valgrind -v --leak-check=full src/globed -printtoconsole
...
# A coverage report will now be accessible at `./test_globe.coverage/index.html`.
```
...
To profile a running globed process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep globed` -- sleep 60
```
...
Globe Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/globe/globe/issues/12691)
...
- [Main thread (`globed`)](https://doxygen.globecore.org/globed_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `globed.cpp`. Responsible for starting up and
  shutting down the application.

- [ThreadImport (`b-loadblk`)](https://doxygen.globecore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
  : Loads blocks from `blk*.dat` files or `-loadblock=<file>` on startup.

- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.globecore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.globecore.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.globecore.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.globecore.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.globecore.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.globecore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.globecore.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.globecore.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.globecore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.globecore.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.globecore.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.globecore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
However, in open source software such as Globe Core, where everyone uses
...
pay attention to for reviewers of Globe Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Globe Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `GLOBE_FOO_BAR_H`, e.g.

```c++
#ifndef GLOBE_FOO_BAR_H
#define GLOBE_FOO_BAR_H
...
#endif // GLOBE_FOO_BAR_H
```
...
Some of these are maintained by active developers of Globe Core, in which case
...
a Globe Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/globe-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/globe-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/globe-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/globe-core/ctaes ; maintained by Core contributors.
...
file descriptors, it will cause problems with Globe's `select()` loop, because
...
compatibility between nodes. This happened in Globe 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Globe's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/globe/globe/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/globe/globe/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/globe/globe/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `globe-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `globe-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Globe Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/globe/globe/pull/14635).
