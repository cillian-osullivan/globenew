# CJDNS support in Globe Core

It is possible to run Globe Core over CJDNS, an encrypted IPv6 network that
...
done outside of Globe Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Globe network and individual nodes.
...
before it will work with your Globe Core node. This is described in steps
...
## Run Globe Core with CJDNS

Once you are connected to the CJDNS network, the following Globe Core
...
When enabled, this option tells Globe Core that it is running in an
...
IPv6 local network. This helps Globe Core perform better address management:
...
CJDNS support was added to Globe Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `globe-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/globe/globe/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Globe Core

There are several ways to see your CJDNS address in Globe Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `globe-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `globe-cli getpeerinfo`).
