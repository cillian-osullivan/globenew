# Fuzzing Globe Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Globe Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/globe/globe/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Globe Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/globe_report.pdf) includes an external overview of the status of Globe Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/globe/globe/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/globe/globe/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/globe/globe/tree/master/src/test/fuzz).
...
It is possible to specify `globed` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`globe-core/qa-assets`](https://github.com/globe-core/qa-assets) repo.

To fuzz `process_message` using the [`globe-core/qa-assets`](https://github.com/globe-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/globe-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/globe-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`globe-core/qa-assets`](https://github.com/globe-core/qa-assets) repo.

Every single pull request submitted against the Globe Core repo is automatically tested against all inputs in the [`globe-core/qa-assets`](https://github.com/globe-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Globe Core more robust.
...
with certain assembly code from Globe Core's code. See [developer notes on sanitizers](https://github.com/globe/globe/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Globe Core using afl++

## Quickstart guide

To quickly get started fuzzing Globe Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Globe Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Globe Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Globe Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Globe
Core without having to write any custom fuzzing harness. The `globed` server
...
```sh
$ mkdir globe-honggfuzz-p2p/
$ cd globe-honggfuzz-p2p/
$ git clone https://github.com/globe/globe
$ cd globe/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/globed.cpp b/src/globed.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/globed -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:18444 -logthreadnames \
                       -debug
```

# Fuzzing Globe Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Globe Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/globe/globe/pull/22472) for more information.
...
Globe Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/globe-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=globe-core).
...
because Globe is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_globe-core/latest).
