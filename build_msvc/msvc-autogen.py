...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Globe Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/globe_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/globe_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Globe-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/globe_config.h'), os.path.join(SOURCE_DIR, 'config/globe-config.h'))
