...
""" Interactive globed P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for Globe Core can enable. It uses BCC
...
def main(globed_path):
    peers = dict()

    globed_with_usdts = USDT(path=str(globed_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    globed_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    globed_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[globed_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/globed")
        exit()
    path = sys.argv[1]
    main(path)
