...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<globe-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<globe-cli args>]`

* `--cmd` lets you customize the globe-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/globe-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `globe-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./globe-cli -conf=mysignet.conf"
