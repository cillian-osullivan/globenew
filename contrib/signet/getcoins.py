...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing globe-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='globe-cli', help='globe-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Globe address to which the faucet should send')
...
parser.add_argument('globe_cli_args', nargs='*', help='Arguments to pass on to globe-cli (default: -signet)')

args = parser.parse_args()

if args.globe_cli_args == []:
    args.globe_cli_args = ['-signet']


def globe_cli(rpc_command_and_params):
    argv = [args.cmd] + args.globe_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = globe_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = globe_cli(['getnewaddress', 'faucet', 'bech32'])
