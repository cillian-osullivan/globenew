...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/globe.conf are parsed in qt/globe.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  globed [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see globed -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for globed but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect globed signal handlers
    noui_connect();
