...
void GlobeDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool GlobeDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule GlobeDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t GlobeDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t GlobeDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t GlobeDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t GlobeDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool GlobeDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void GlobeDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void GlobeDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void GlobeDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data, chainstate)[0].execRes.output;
}

void GlobeDGP::createParamsInstance(){
...
dev::Address GlobeDGP::getAddressForBlock(unsigned int blockHeight){
...
void GlobeDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GlobeDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void GlobeDGP::parseStorageOneUint64(uint64_t& value){
...
void GlobeDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule GlobeDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
