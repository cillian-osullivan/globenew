...
GlobeState::GlobeState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = GlobeState::openDB(_path + "/globeDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

GlobeState::GlobeState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute GlobeState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, GlobeTransaction const& _t, CChain& _chain, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createGlobeAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            globe::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, GlobeTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, GlobeTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> GlobeState::vins() const // temp
{
...
void GlobeState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* GlobeState::vin(dev::Address const& _a) const
{
    return const_cast<GlobeState*>(this)->vin(_a);
}

Vin* GlobeState::vin(dev::Address const& _addr)
{
...
// void GlobeState::commit(CommitBehaviour _commitBehaviour)
// {
...
void GlobeState::kill(dev::Address _addr)
{
...
void GlobeState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void GlobeState::deleteAccounts(std::set<dev::Address>& addrs){
...
void GlobeState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void GlobeState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void GlobeState::validateTransfersWithChangeLog(){
...
void GlobeState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!GlobeState::addressInUse(delegationsAddress)){
        GlobeState::createContract(delegationsAddress);
        GlobeState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, GlobeState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
