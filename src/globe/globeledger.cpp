...
// For details see https://github.com/globe/globe/pull/22348.
...
using namespace GlobeLedger_NS;

class GlobeLedgerPriv
{
public:
    GlobeLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("GlobeLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
GlobeLedger::GlobeLedger():
    d(0)
{
    d = new GlobeLedgerPriv();
}
...
GlobeLedger &GlobeLedger::instance()
{
    static GlobeLedger device;
    return device;
}

bool GlobeLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool GlobeLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool GlobeLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool GlobeLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool GlobeLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool GlobeLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool GlobeLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string GlobeLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool GlobeLedger::toolExists()
{
    return d->toolExists;
}

bool GlobeLedger::isStarted()
{
    return d->fStarted;
}

void GlobeLedger::wait()
{
    if(d->fStarted)
    {
...
bool GlobeLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool GlobeLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool GlobeLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool GlobeLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool GlobeLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool GlobeLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Globe Stake" || device.app_name == "Globe Stake Test";
        if(isStakeApp == stake)
        {
...
bool GlobeLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool GlobeLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool GlobeLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool GlobeLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string GlobeLedger::derivationPath(int type)
{
