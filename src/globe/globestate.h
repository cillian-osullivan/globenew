...
class GlobeTransactionReceipt: public dev::eth::TransactionReceipt {
public:
    GlobeTransactionReceipt(dev::h256 const& state_root, dev::h256 const& utxo_root, dev::u256 const& gas_used, dev::eth::LogEntries const& log) : dev::eth::TransactionReceipt(state_root, gas_used, log), m_utxoRoot(utxo_root) {}
...
class GlobeState : public dev::eth::State {
    
public:

    GlobeState();

    GlobeState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, GlobeTransaction const& _t, CChain& _chain, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createGlobeAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<GlobeState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<GlobeState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(GlobeState* _state, const std::vector<TransferInfo>& _transfers, const GlobeTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const GlobeTransaction& transaction;
