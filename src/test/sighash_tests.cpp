...
{
    tx.nVersion = ((uint32_t)InsecureRand32()) % (GLOBE_TXN_VERSION-1);
    tx.vin.clear();
    tx.vout.clear();
...
BOOST_AUTO_TEST_CASE(sighash_from_data)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        bool fExpectHashFailure = false; // adjusting test vectors >= GLOBE_TXN_VERSION
        try {
          ...
          sigHashHex = test[4].get_str();
            ...
            strHex[0] = raw_tx[0];
            strHex[1] = raw_tx[1];
            if (std::strtoul(strHex, nullptr, 16) >= GLOBE_TXN_VERSION)
            {
                raw_tx[0] = '0';
                raw_tx[1] = '0';
