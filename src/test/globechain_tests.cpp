...
BOOST_FIXTURE_TEST_SUITE(globechain_tests, GlobeBasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(signature_test)
{
    ...
    txn2.nVersion = GLOBE_TXN_VERSION;
...
BOOST_AUTO_TEST_CASE(globechain_test)
{
    SeedInsecureRand();
    FillableSigningProvider keystore;
    ...
    CBlock blk;
    blk.nVersion = GLOBE_BLOCK_VERSION;
    blk.nTime = 1487406900;
...
BOOST_AUTO_TEST_CASE(mixed_input_types)
{
    CMutableTransaction txn;
    txn.nVersion = GLOBE_TXN_VERSION;
    BOOST_CHECK(txn.IsGlobeVersion());
    ...
    txnPrev.nVersion = GLOBE_TXN_VERSION;
    BOOST_CHECK(txnPrev.IsGlobeVersion());
...
BOOST_AUTO_TEST_CASE(mixed_output_types)
{
    ...
    CTransaction tx_c(txn);
    TxValidationState state;
    state.SetStateInfo(GetTime(), nSpendHeight, Params().GetConsensus(), true /* globe_mode */, false /* skip_rangeproof */);
