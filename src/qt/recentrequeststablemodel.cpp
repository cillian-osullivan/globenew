    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return GlobeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, GlobeUnits::SeparatorStyle::NEVER);
            else
                return GlobeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    return tr("Requested") +
           QLatin1String(" (") +
           GlobeUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) +
           QLatin1Char(')');
}
