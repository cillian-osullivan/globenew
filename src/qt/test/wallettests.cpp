...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<GlobeAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = GlobeUnits::formatWithUnit(unit, expected_balance, false, GlobeUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_globe-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_globe-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_globe-qt  # macOS
void TestGUI(interfaces::Node& node)
{
    return; // Globe TODO: convert to CHDWallet or modify gui code to allow CWallet again
    ...
    // Amount input
    GlobeAmountField* amountInput = receiveCoinsDialog.findChild<GlobeAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_globe-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
