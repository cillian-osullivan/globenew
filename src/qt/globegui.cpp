...
const std::string GlobeGUI::DEFAULT_UIPLATFORM =
...
GlobeGUI::GlobeGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &GlobeGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &GlobeGUI::showModalOverlay);
#ifdef ENABLE_WALLET
    if (enableWallet) {
        connect(labelWalletEncryptionIcon, &GUIUtil::ClickableLabel::clicked, this, &GlobeGUI::toggleLockState);
    }
#endif
...
void GlobeGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Globe address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and globe: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &GlobeGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &GlobeGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &GlobeGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Globe addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Globe addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Globe Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Globe Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a globe: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Globe command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &GlobeGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &GlobeGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &GlobeGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &GlobeGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &GlobeGUI::showDebugWindow);
    connect(mnemonicAction, &QAction::triggered, this, &GlobeGUI::showMnemonicClicked);
    connect(coldstakingAction, &QAction::triggered, this, &GlobeGUI::showColdStakingClicked);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &GlobeGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &GlobeGUI::setCurrentWallet);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &GlobeGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &GlobeGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &GlobeGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &GlobeGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &GlobeGUI::showDebugWindow);
}

void GlobeGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void GlobeGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &GlobeGUI::setCurrentWalletBySelectorIndex);
...
void GlobeGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &GlobeGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &GlobeGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &GlobeGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::waitingForDevice, this, &GlobeGUI::waitingForDevice);

        // Show progress dialog
        connect(_clientModel, &ClientModel::showProgress, this, &GlobeGUI::showProgress);
...
#ifdef ENABLE_WALLET
void GlobeGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &GlobeGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &GlobeGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* GlobeGUI::getWalletController()
{
    return m_wallet_controller;
}

void GlobeGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &GlobeGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &GlobeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &GlobeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &GlobeGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &GlobeGUI::incomingTransaction);
    connect(this, &GlobeGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void GlobeGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void GlobeGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if(!walletFrame)
        return;
...
void GlobeGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void GlobeGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GlobeGUI::setWalletActionsEnabled(bool enabled)
{
...
void GlobeGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &GlobeGUI::toggleHidden);
...
void GlobeGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void GlobeGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void GlobeGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void GlobeGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void GlobeGUI::showMnemonicClicked()
{
#ifdef ENABLE_WALLET
    if (!walletFrame) {
        return;
...
void GlobeGUI::showColdStakingClicked()
{
#ifdef ENABLE_WALLET
    if (!walletFrame) {
        return;
...
void GlobeGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void GlobeGUI::openClicked()
{
...
void GlobeGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GlobeGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GlobeGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GlobeGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GlobeGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GlobeGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void GlobeGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void GlobeGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Globe network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void GlobeGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GlobeGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void GlobeGUI::updateHeadersSyncProgressLabel()
{
...
void GlobeGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void GlobeGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &GlobeGUI::quitRequested);
...
void GlobeGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void GlobeGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void GlobeGUI::waitingForDevice(bool fCompleted)
{
    if (!fCompleted) {
...
void GlobeGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &GlobeGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &GlobeGUI::show);
                e->ignore();
            }
        }
    }
...
void GlobeGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GlobeGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GlobeGUI::incomingTransaction(const QString& date, GlobeUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(GlobeUnits::formatWithUnit(unit, amount, true));
...
void GlobeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GlobeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GlobeGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GlobeGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GlobeGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void GlobeGUI::setEncryptionStatus(int status)
{
...
void GlobeGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void GlobeGUI::updateWindowTitle()
{
...
void GlobeGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GlobeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GlobeGUI::detectShutdown()
{
...
void GlobeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void GlobeGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GlobeGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = GlobeGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void GlobeGUI::subscribeToCoreSignals()
{
...
void GlobeGUI::unsubscribeFromCoreSignals()
{
...
bool GlobeGUI::isPrivacyModeActivated() const
{
      ...
      m_platform_style{platformStyle}
{
    ...
    QList<GlobeUnit> units = GlobeUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GlobeUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, GlobeUnits::longName(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu()
{
    ...
    for (const GlobeUnit u : GlobeUnits::availableUnits()) {
        menu->addAction(GlobeUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(GlobeUnit newUnits)
{
    setText(GlobeUnits::longName(newUnits));
}
