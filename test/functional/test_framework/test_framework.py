...
TMPDIR_PREFIX = "globe_func_test_"
...
class GlobeTestMetaClass(type):
    """Metaclass for GlobeTestFramework.

    Ensures that any attempt to register a subclass of `GlobeTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'GlobeTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("GlobeTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("GlobeTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class GlobeTestFramework(metaclass=GlobeTestMetaClass):
    """Base class for a globe test script.

    Individual globe test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave globeds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop globeds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use globe-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_globed = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globed" + config["environment"]["EXEEXT"],
        )
        fname_globecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globe-cli" + config["environment"]["EXEEXT"],
        )
        fname_globeutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "globe-util" + config["environment"]["EXEEXT"],
        )
        self.options.globed = os.getenv("GLOBED", default=fname_globed)
        self.options.globecli = os.getenv("GLOBECLI", default=fname_globecli)
        self.options.globeutil = os.getenv("GLOBEUTIL", default=fname_globeutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: globeds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("GlobeRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'globed', self.options.globed) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'globe-cli', self.options.globecli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.globeconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a globed test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple globed test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as globed's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    globed=self.options.globed,
                    globe_cli=self.options.globecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in globe.conf
    ...
    def skip_if_no_globed_tracepoints(self):
        """Skip the running test if globed has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("globed has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_globed_zmq(self):
        """Skip the running test if globed has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("globed has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if globe-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("globe-wallet has not been compiled")

    def skip_if_no_globe_util(self):
        """Skip the running test if globe-util has not been compiled."""
        if not self.is_globe_util_compiled():
            raise SkipTest("globe-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if globe-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("globe-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether globe-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether globe-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_globe_util_compiled(self):
        """Checks whether globe-util was compiled."""
        return self.config["components"].getboolean("ENABLE_GLOBE_UTIL")
