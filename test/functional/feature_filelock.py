...
"""Check that it's not possible to start a second globed instance using the same datadir or wallet."""
import os
import random
import string
...
class FilelockTest(GlobeTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second globed instance using the same datadir")
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second globed instance using the same wallet")
                if descriptors:
