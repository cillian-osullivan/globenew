...
class HTTPBasicsTest(GlobeTestFramework):
    ...
    def setup_chain(self):
        super().setup_chain()
        #Append rpcauth to globe.conf before initialization
        ...
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 0), "globe.conf"), 'a', encoding='utf8') as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(os.path.join(get_datadir_path(self.options.tmpdir, 1), "globe.conf"), 'a', encoding='utf8') as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
