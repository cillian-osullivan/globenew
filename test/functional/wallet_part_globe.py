...
class WalletGlobeTest(GlobeTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info('Test globe-wallet')
        out = textwrap.dedent('''\
            Wallet info
            ===========
            Name: w_created
        ...
        p = self.globe_wallet_process('-h', 'generatemnemonic')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
        assert ('1. language' in stdout)
        p = self.globe_wallet_process('generatemnemonic')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
        assert (len(stdout.split(' ')) == 24)

        p = self.globe_wallet_process('generatemnemonic', 'spanish', '16')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
